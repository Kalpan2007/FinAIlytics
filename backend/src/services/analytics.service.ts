...
export const summaryAnalyticsService = async (
  ...
  return {
    ...
    savingRate: {
      percentage: (() => {
        const pct = parseFloat(savingData.savingsPercentage.toFixed(2));
        // clamp displayed percentage to a realistic range
        return Math.max(Math.min(pct, 100), -100);
      })(),
      expenseRatio: (() => {
        const ratio = parseFloat(savingData.expenseRatio.toFixed(2));
        // ensure 0-100 range for ratio
        return Math.max(Math.min(ratio, 100), 0);
      })(),
    },
    transactionCount,
    percentageChange: {
